...
public class MediaTopology {
    ...
    private void videosCount(KStream<String, VideoRecord> generalOrdersStream) {
        ...
        KTable<String, Long> videosCount = generalOrdersStream.map(
                ...
                .count(Named.as(MediaTopology.VIDEOS_COUNT),
                        //use state store to save data
                        Materialized.as(MediaTopology.VIDEOS_COUNT));

        videosCount.toStream()
                .print(Printed.<String, Long>toSysOut().withLabel(MediaTopology.VIDEOS_COUNT));
